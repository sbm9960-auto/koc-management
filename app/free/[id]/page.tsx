...
export default function FreePostDetail() {
  ...
  useEffect(() => {
    ...
    setPosts(getLS<Post[]>(LS.postsFree, []));
    const cs = getLS<CommentItem[]>(LS.comments, []);
    setComments(cs);
  }, []);
  ...
  useEffect(() => {
    ...
    setPosts(updated);
    setLS(LS.postsFree, updated);
  // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [postId]);

  // comments
  const postComments = useMemo(
    () => comments.filter(c => c.postId === postId && c.board === 'free'),
    [comments, postId]
  );
  ...
  const removeComment = (id: number) => {
    ...
    // decrease count
    if (post) {
      const updatedPosts = posts.map(p => p.id === post.id ? { ...p, comments: Math.max(0, p.comments - 1) } : p);
      setPosts(updatedPosts);
      setLS(LS.postsFree, updatedPosts);
    }
  };
  ...
  if (!post) {
    return (
      <div style={{ padding: 24 }}>
        <p>게시글을 찾을 수 없습니다.</p>
        <button onClick={() => router.push('/?menu=board')} style={{ marginTop: 12 }}>← 목록으로</button>
      </div>
    );
  }
  ...
  return (
    <div style={{ maxWidth: 900, margin: '24px auto', padding: '0 16px' }}>
      <button onClick={() => router.push('/?menu=board')} style={{ marginBottom: 12 }}>← {t.freeBoard}</button>
